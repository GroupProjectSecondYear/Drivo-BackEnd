		...
		if (result != null) {
			...
			for (int n = 1; n < answers.length; n++) { // split array of answers
				...
				if (ans.length() == paper.getNo_of_answers()) {
					...
					// **if (replyQ == null) {
					// **questionRepository.deleteQuestionsOfaPaper(result.getPaperId()); // delete
					// all questions if
		...
		if (paperRepository.existsById(paper.getPaperId())) {
			...
			if (paper.getNo_of_answers() != savedPaper.getNo_of_answers()) {
				...
				if (paper.getNo_of_answers() > savedPaper.getNo_of_answers()) {
					...
					// questionRepository. addExtraAnswers(paper.getPaperId(),
					// paper.getNo_of_questions(),val);//bcs of a miss working behaviour
