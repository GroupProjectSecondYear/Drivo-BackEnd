...
public interface SalaryRepository extends JpaRepository<Salary, Integer> {
	...
	@Query(value = "select * from salary s,staff e,user u where " + "s.staff_id=e.staff_id and "
			+ "e.user_id=u.user_id and " + "u.status = 1 and "
			+ "s.month = :month and s.year = :year order by s.complete asc,u.role asc,s.total_payment desc", nativeQuery = true)
