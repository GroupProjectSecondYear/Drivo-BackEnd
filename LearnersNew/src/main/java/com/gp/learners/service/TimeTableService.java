...
public class TimeTableService {
	...
	public List<InstructorMap> getRelevantInstructors(Integer dayId, Integer packageId, Integer timeSlotId,
			Integer pathId, Integer transmission) {
		...
		if (dayId > -1 && dayId < 7 && transmission > 0 && transmission < 3) {
			if (dayId != null && packageId != null && timeSlotId != null && transmission != null && pathId != null) {
				if (packageRepository.existsById(packageId) && timeSlotRepository.existsById(timeSlotId)
						&& pathRepository.existsById(pathId)) {
					List<Integer> instructorIdList = instructorRepository.getRelevantInstructors(dayId, packageId,
							timeSlotId, transmission);
					...
					for (Integer id : instructorIdList) {
						...
						instructorList.add(new InstructorMap(id, instructor.getStaffId().getUserId().getFirstName()
								+ " " + instructor.getStaffId().getUserId().getLastName()));
					}
	...
	// Lesson Functions
	public String addLesson(Integer dayId, Integer packageId, Integer timeSlotId, Integer pathId, Integer transmission,
			Integer instructorId, Integer numStudent) {
		if (dayId > -1 && dayId < 7 && transmission > 0 && transmission < 3 && numStudent > 0) {
			if (packageRepository.existsById(packageId) && timeSlotRepository.existsById(timeSlotId)
					&& pathRepository.existsById(pathId) && instructorRepository.existsById(instructorId)) {
			...
			if (lessons != null && lessons.size() > 0) {
				...
				for (Lesson lesson : lessons) {
					if (timeSlot.equals(lesson.getTimeSlotId())) {
						...
						packageDataRow.add(lesson.getPackageId().getTitle() + "("
								+ getTransmission(lesson.getTransmission()) + ")");
	...
	public String lessonDeactivate(Integer lessonId) {
		if (lessonRepository.existsById(lessonId)) {
			...
			List<StudentLesson> studentLessonList = studentLessonRepository
					.findNotificationStudentListByLessonId(lessonRepository.findByLessonId(lessonId));
	...
	public Integer lessonActivate(Integer lessonId) {
		if (lessonRepository.existsById(lessonId)) {
			...
			Lesson object = lessonRepository.findByDeactivateLessonDetails(deactivateLesson.getDay(),
					deactivateLesson.getTimeSlotId(), deactivateLesson.getInstructorId());
	...
	public String updateLesson(Integer lessonId, Integer type, Integer dayId, Integer timeSlotId, Integer pathId,
			Integer instructorId, Integer numStudent) {
		if (dayId > -1 && dayId < 7 && numStudent > 0) {
			if (lessonRepository.existsById(lessonId) && timeSlotRepository.existsById(timeSlotId)
					&& pathRepository.existsById(pathId) && instructorRepository.existsById(instructorId)) {
				Lesson lesson = lessonRepository.findByLessonId(lessonId);

				// Create Update LessonObject for notification Purpose
				Lesson updateLesson = new Lesson(lessonId, dayId, -1, numStudent, -1, lesson.getDate(),
						instructorRepository.findByInstructorId(instructorId), lesson.getPackageId(),
						timeSlotRepository.findByTimeSlotId(timeSlotId), pathRepository.findByPathId(pathId));
	...
	public List<LessonDistributionMap> getLessonDistributionDetails(Integer packageId, Integer transmission) {
		...
		if (packageRepository.existsById(packageId) && transmission > 0 && transmission < 3) {
			lessonDistribution = lessonRepository
					.findByPackageIdAndTransmission(packageRepository.findByPackageId(packageId), transmission);
	...
	public List<PackageAnalysisDataMap> getLessonsByPackageId(Integer packageId, Integer transmission) {
		...
		while (i <= 7) {
			...
			if (i == 7) {
				lessonList = lessonRepository.getLessonsASCByPackageId(0, 1,
						packageRepository.findByPackageId(packageId), transmission);
			} else {
				lessonList = lessonRepository.getLessonsASCByPackageId(i, 1,
						packageRepository.findByPackageId(packageId), transmission);
			}
	...
	public Integer isAnyLesson(Integer packageId, Integer transmission) {
		if (packageRepository.existsById(packageId)) {
			List<LessonDistributionMap> object = lessonRepository
					.findByPackageIdAndTransmission(packageRepository.findByPackageId(packageId), transmission);
	...
	public List<TimeSlot> getTimeSlotListAccordingToDateAndPackage(Integer packageId, Integer transmission,
			LocalDate date) {

		/*
		 * In the db store 0->Sunday 1->Monday 6->Saturday
		 * 
		 * But getDayOfWeek function give as 1-> Monday 7->Sunday
		 * 
		...
		if (packageRepository.existsById(packageId)) {
			List<Lesson> lessonList = lessonRepository.getTimeSlotListAccordingToDateAndPackage(dayId,
					packageRepository.findByPackageId(packageId), transmission);
			List<TimeSlot> timeSlotList = new ArrayList<TimeSlot>();
			for (Lesson lesson : lessonList) {
				TimeSlot object = lesson.getTimeSlotId();
				if (object != null && !timeSlotList.contains(object)) {
					timeSlotList.add(object);
				}
			}
			return timeSlotList;
		}
	...
	private Boolean notExistFromAndTo(TimeSlot timeSlot) {
		TimeSlot object = timeSlotRepository.findByStartTimeandFinishTime(timeSlot.getStartTime(),
	...
	private Boolean notExistLesson(Lesson lesson) {
		Lesson object = lessonRepository.findByLessonDetails(lesson.getDay(), lesson.getTransmission(),
				lesson.getNumStu(), lesson.getInstructorId(), lesson.getTimeSlotId(), lesson.getPackageId(),
				lesson.getPathId());
		if (object != null) {
			return false;
		}
