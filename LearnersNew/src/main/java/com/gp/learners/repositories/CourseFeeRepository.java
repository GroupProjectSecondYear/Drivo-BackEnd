...
public interface CourseFeeRepository extends JpaRepository<CourseFee, Integer>{
	...
	@Query(value = "select sum(amount) from course_fee  where student_package_id = :studentPackageId",nativeQuery=true)
	public Double findFeeForPackageByStudentIdAndPackageId(Integer studentPackageId);
}
